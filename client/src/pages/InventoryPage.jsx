...
const InventoryPage = () => {
  ...
  const applyFilters = (term, currentFilters) => {
    ...
    const filtered = data.cars.filter((car) => {
      ...
      const matchesYear =
        car.year >= currentFilters.yearRange[0] &&
        car.year <= currentFilters.yearRange[1];
      const matchesMake =
        currentFilters.make.length === 0 ||
        currentFilters.make.includes(car.make);
      const matchesMileage =
        car.mileage >= currentFilters.mileageRange[0] &&
        car.mileage <= currentFilters.mileageRange[1];
      const matchesTransmission =
        currentFilters.transmission.length === 0 ||
        currentFilters.transmission.includes(car.transmission);
